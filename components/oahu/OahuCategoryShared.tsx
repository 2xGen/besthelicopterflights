...
import type { OahuCategory } from "@/data/oahu-categories";
import { getOahuRankedTours } from "@/data/oahu-categories";
import { VIATOR_LINK_REL, viatorAffiliateUrl } from "@/lib/viator";

type Props = {
  category: OahuCategory;
};

export function OahuCategoryQuickTable({
  category,
  footnote,
}: Props & { footnote?: string }) {
  const rows = getOahuRankedTours(category);
...
export function OahuCategoryRankings({ category }: Props) {
  const rows = getOahuRankedTours(category);
...
export function OahuCategoryChooseGuide({ category }: Props) {
  const rows = getOahuRankedTours(category);
