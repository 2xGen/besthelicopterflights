import { featuredBigIslandTours } from "@/data/big-island-tours";
import { VIATOR_LINK_REL, viatorAffiliateUrl } from "@/lib/viator";

export function BigIslandComparisonTable() {
  return (
    <section
      id="comparison"
    ...
    >
      <div className="mx-auto w-full min-w-0 max-w-6xl">
        ...
        <h2 className="mt-4 font-display text-3xl leading-tight text-navy md:text-4xl">
          Best Big Island helicopter tours
        </h2>
        <p className="mt-5 max-w-3xl text-base leading-relaxed text-stone md:text-lg">
          ...
          clarity (doors-on/off, private, landings), departure base (Kona,
          Waikoloa or Hilo) and value relative to experience — not Viator’s
          default listing order. “Best for” labels explain why each tour earns
          its place.
        </p>

        <ul className="mt-10 space-y-3 md:hidden">
          {featuredBigIslandTours.map((tour) => (
            <li
              key={tour.id}
        ...
        <p className="mt-4 text-xs leading-relaxed text-stone-light">
          ...
          at research time and can change. Active-lava or glow viewing is never
          guaranteed. We may earn a commission when you book through Viator
          links on this page.
        </p>
      </div>
    </section>
