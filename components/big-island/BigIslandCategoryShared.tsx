...
import type { BigIslandCategory } from "@/data/big-island-categories";
import { getBigIslandRankedTours } from "@/data/big-island-categories";
import { VIATOR_LINK_REL, viatorAffiliateUrl } from "@/lib/viator";

type Props = {
  category: BigIslandCategory;
};

export function BigIslandCategoryQuickTable({
  category,
  footnote,
}: Props & { footnote?: string }) {
  const rows = getBigIslandRankedTours(category);
...
export function BigIslandCategoryRankings({ category }: Props) {
  const rows = getBigIslandRankedTours(category);
          ...
          {rows.map(({ tour, rank, award, why, chooseIf, relatedLink }) => (
            ...
            >
              ...
              <div className="relative z-10 pointer-events-none">
                ...
                <p className="mt-5 text-base leading-relaxed text-stone">{why}</p>
                {relatedLink && (
...
export function BigIslandCategoryChooseGuide({ category }: Props) {
  const rows = getBigIslandRankedTours(category);
