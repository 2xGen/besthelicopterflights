...
import type { MauiCategory } from "@/data/maui-categories";
import { getMauiRankedTours } from "@/data/maui-categories";
import { VIATOR_LINK_REL, viatorAffiliateUrl } from "@/lib/viator";

type Props = {
  category: MauiCategory;
};

export function MauiCategoryQuickTable({
  category,
  footnote,
}: Props & { footnote?: string }) {
  const rows = getMauiRankedTours(category);
...
export function MauiCategoryRankings({ category }: Props) {
  const rows = getMauiRankedTours(category);
...
export function MauiCategoryChooseGuide({ category }: Props) {
  const rows = getMauiRankedTours(category);
